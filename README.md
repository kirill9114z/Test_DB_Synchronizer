...
В репозиторий не нужно добавлять реальный `.env` файл, так как он может содержать доступы к базам данных.

Нужно создать `.env` локально на основе `.env.example`.
...
- dry-run режим с сохранением SQL в файл;
- логирование в файл;
- поддержка индексов и внешних ключей;
- подтверждение перед execute;
- Docker Compose для локального поднятия двух PostgreSQL баз;
...
Этот проект реализует безопасный инструмент синхронизации схемы PostgreSQL между development и production окружениями.

Основная цель решения — аккуратно переносить недостающие изменения схемы в production БД без риска повредить существующие данные.
